...
//APIs Global Stats
// blockchain.info /stats returns difficulty + hash_rate (GH/s) + a few other
// scalars in ~1 KB. Picked over mempool.space endpoints because:
//   - /api/v1/mining/hashrate/3d aggregates 3 days of samples (~50 KB,
//     server-side compute, can take 30–60 s)
//   - /api/v1/blocks returns 15 latest blocks (~10–30 KB)
//   - blockchain.info answers in well under a second.
#define getGlobalHash "https://api.blockchain.info/stats"
...
// Cached coin/network stats — exposed so axehub_api can clear them on coin
// switch (otherwise stale values from the previous coin linger until the
