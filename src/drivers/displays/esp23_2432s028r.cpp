...
#if (defined(ESP32_2432S028R) || defined(ESP32_2432S028_2USB)) && !defined(AXEHUB_DISPLAY)
...
// Pre-allocated dedicated sprites — never share buffers with `background`
// to avoid stale-pixel bleed-through after smaller sprites reuse the memory.
TFT_eSprite poolSprite = TFT_eSprite(&tft);
TFT_eSprite minerRightSprite = TFT_eSprite(&tft);
SPIClass hSPI(HSPI);
...
{ 
  ...
  if (poolSprite.created()) {
    poolSprite.setColorDepth(16);
...
// On heap fragmentation falls back to 8x8 to avoid null-pointer derefs in
// the caller's draw calls.
bool createBackgroundSprite(int16_t wdt, int16_t hgt) {
  background.createSprite(wdt, hgt);
  if (!background.created()) {
    Serial.printf("[Sprite] alloc fail W:%d H:%d\n", wdt, hgt);
    printheap();
    background.createSprite(8, 8);
    if (background.created()) {
...
void printPoolData(){
  if ((hasChangedScreen) || (mPoolUpdate == 0) || (millis() - mPoolUpdate > UPDATE_POOL_min * 60 * 1000)){
      if (Settings.PoolAddress != "tn.vkbit.com") {
          pData = getPoolData();
          if (!poolSprite.created()) {
            Serial.println("###### POOL SPRITE NOT ALLOCATED ######");
            printheap();
            return;
          }
          uint16_t bandBg = bottomScreenBlue ? 0x18EB : 0x1A21;  // dark blue / dark olive
          const uint16_t* poolSrc = bottomScreenBlue ? bottonPoolScreen : bottonPoolScreen_g;
          poolSprite.pushImage(0, 0, 320, 50, poolSrc + 20 * 320);  // skip top 20 rows
          // Re-draw the bitmap top strip so "Best Difficulty" / "Total
          // Hash Rate" labels (in the side zones of the artwork) are intact.
          if (bottomScreenBlue)
            tft.pushImage(0, 170, 320, 20, bottonPoolScreen);
          else
            tft.pushImage(0, 170, 320, 20, bottonPoolScreen_g);
          // Overpaint the middle slot of the bitmap top strip with the
          // configured pool address (chamfered bottom corners).
          const int bandX = 106, bandW = 109;
          const int bandY = 170, bandH = 20;
          const int chamfer = 4;
          tft.fillRect(bandX + chamfer, bandY, bandW - 2 * chamfer, bandH, bandBg);
          tft.fillRect(bandX, bandY, chamfer, bandH - chamfer, bandBg);
          tft.fillRect(bandX + bandW - chamfer, bandY, chamfer, bandH - chamfer, bandBg);
          for (int i = 0; i < chamfer; i++) {
            int rowY = bandY + bandH - chamfer + i;
            int stepW = chamfer - i - 1;
            if (stepW > 0) {
              tft.fillRect(bandX + chamfer - stepW, rowY, stepW, 1, bandBg);
              tft.fillRect(bandX + bandW - chamfer, rowY, stepW, 1, bandBg);
            }
          }
          tft.setTextColor(TFT_WHITE, bandBg);
          tft.setTextDatum(MC_DATUM);
          tft.setTextFont(1);
          tft.setTextSize(1);
          String label = Settings.PoolAddress;
          int colon = label.indexOf(':');
          if (colon > 0) label = label.substring(0, colon);
          while (tft.textWidth(label) > bandW - 4 && label.length() > 4) {
            label.remove(label.length() - 1);
          }
          tft.drawString(label, bandX + bandW / 2, 180);
...
void esp32_2432S028R_MinerScreen(unsigned long mElapsed)
{
  ...
  // Sprite 2 — hashrate (height 65 fits font 35 without overflow).
  createBackgroundSprite(WIDTH-7, 65);
...
// Absolute screen coordinates of the coin-ticker icon on the bitmap.
// The second sprite of each screen starts at y=3, so any rows above that
// (kCoinIconY..2) must be painted directly on tft — but only ONCE per screen
// change since the sprite never touches those rows. Rows >= 3 are drawn into
// the sprite so they update atomically with the dynamic content (no flicker).
static const int kCoinIconX = 179;
...
static void drawCoinTickerOnSprite(int spriteOriginX, int spriteOriginY) {
  ...
  const int cy = sy + 8;
  const int radius = 8;
  uint16_t fillColor = 0xFD20;
  background.fillCircle(cx, cy, radius, fillColor);
  background.drawCircle(cx, cy, radius, TFT_WHITE);
  background.setTextColor(TFT_WHITE, fillColor);
  background.setTextDatum(MC_DATUM);
  background.setTextFont(Settings.CoinTicker.length() <= 3 ? 2 : 1);
  background.setTextSize(1);
  background.drawString(Settings.CoinTicker.c_str(), cx, cy);
}

// Paint ONLY the rows of the icon that lie above the second sprite (i.e. the
// area sprites never touch). Called once when hasChangedScreen=true, right
// after the full-screen bitmap pushImage.
static void drawCoinTickerOverlay() {
  if (Settings.CoinTicker == "BTC") return;
...
void esp32_2432S028R_ClockScreen(unsigned long mElapsed)
{
  ...
  if (hasChangedScreen) {
    ...
    drawCoinTickerOverlay();   // top rows above sprite area, painted once
  }
 ...
 // Create background sprite to print data at once
  ...
  background.deleteSprite();   createBackgroundSprite(169,105);
  // Print background screen
  ...
  // Coin ticker logo — drawn ONTO the sprite so it goes out atomically
  // with the rest of the sprite (no flicker between sprite push and overlay).
  drawCoinTickerOnSprite(130, 3);

  // Push prepared background to screen
  background.pushSprite(130, 3);

  // Delete sprite to free the memory heap
  background.deleteSprite();   Serial.printf(">>> Completed %s share(s), %s Khashes, avg. hashrate %s KH/s\n",
                data.completedShares.c_str(), data.totalKHashes.c_str(), data.currentHashRate.c_str());
...
void esp32_2432S028R_GlobalHashScreen(unsigned long mElapsed)
{
  ...
  // Coin ticker logo — drawn onto the sprite so it goes out atomically.
  drawCoinTickerOnSprite(160, 3);
  // Push prepared background to screen
  background.pushSprite(160, 3);
  // Delete sprite to free the memory heap
  background.deleteSprite();   // Create background sprite to print data at once
  ...
  // Re-draw on top of any sprite that may have overlapped the icon area.
  drawCoinTickerOverlay();
...
void esp32_2432S028R_BTCprice(unsigned long mElapsed)
{
 ...
 // Create background sprite to print data at once
  ...
  background.pushSprite(0, 130);
  // Delete sprite to free the memory heap
  background.deleteSprite();   createBackgroundSprite(169,90);
  // Print background screen
